#!/bin/python
"""
Script for generating contour/quiver plots of wave height and/or wind speed,
using output from a SWAN model run.

Example run:
python swan_plots.py --model-output Spatial.mat --basemap PugetSound150.mat --wave-plot
"""
...
def wave_height_plot(W, T):
    ...
    im = plt.contourf(W['Xp'], W['Yp'], W['Hsig']*3.28, wave_contours, cmap=color_map, vmin=min(wave_contours), vmax=max(wave_contours))
    ...
    thin = 15
    wave_dir = 90 - W['Dir'] + 180
    wave_X = W['Xp'][::thin,::thin]
    wave_Y = W['Yp'][::thin,::thin]
